...
export class HistoricalDataService {
  ...
  private async generateMockData(
    ...
    for (let i = 0; i < totalPoints; i++) {
      ...
      const meanReversion = (basePrice - currentPrice) * 0.001 // Pull back to mean
      const trend = trendBias

      currentPrice *= (1 + randomWalk + meanReversion + trend)
      ...
      const volume = (baseVolume * volumeMultiplier).toString()
